...
| 023 | `023_google_tasks_sync.sql` | **[Feature]** Sync bidirectionnelle `taches` ↔ Google Tasks (admin uniquement). Ajoute `google_task_id`/`google_etag`/`synced_at` sur `taches` + table `google_oauth_state` mono-ligne. Voir `023_README.md` pour la conf Google Cloud Console et les env vars. |
