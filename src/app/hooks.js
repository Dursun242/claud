  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('chantiers')
        .select('id,nom,adresse,commune,client,budget,created_at,updated_at')
...
export function useContacts() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('contacts')
        .select('id,nom,type,specialite,tel,email,siret')
        .order('nom')
        .limit(50);
      if (error) throw error;
...
export function useTasks() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('taches')
        .select('id,chantier_id,titre,description,urgence,created_at')
        .order('created_at', { ascending: false })
        .limit(30);
      if (error) throw error;
