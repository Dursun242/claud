...
export default function OrdresServiceV({data,m,reload}) {
  ...
  const handleSendSign = async () => {
    ...
    setSignError("");
    try {
      // Générer le PDF de l'OS en base64
      ...
      if (!pdfBase64) throw new Error("Impossible de générer le PDF de l'OS");
      ...
      {signModal && (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:8,padding:10,fontSize:12,color:"#166534"}}>
            Le PDF de l'OS sera généré et envoyé directement à Odoo Sign. {signModal.artisan_nom} recevra un email pour signer électroniquement.
          </div>
