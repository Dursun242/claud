...
export default function TasksV({data,save,m,reload}) {
  ...
  const tasks = filter==="all"?allTasks:allTasks.filter(t=>t.statut===filter);
  const openNew=()=>{setForm({chantierId:chantiers[0]?.id||"",titre:"",priorite:"En cours",statut:"Planifié",echeance:"",lot:""});setModal("new");};
  ...
  return (<div>
    ...
    <div style={{display:"grid",gap:6}}>
      {tasks.map(t=>{const ch=Array.isArray(chantiers) ? chantiers.find(c=>c.id===t.chantierId) : null;return(
