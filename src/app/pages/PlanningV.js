...
export default function PlanningV({data,m}) {
  ...
  const items = filter==="all"?planning:planning.filter(p=>p.chantierId===filter||p.chantier_id===filter);
  ...
  const timestamps = planning.map(p=>new Date(p.debut).getTime()).filter(t=>!isNaN(t));
  const endTimestamps = planning.map(p=>new Date(p.fin).getTime()).filter(t=>!isNaN(t));
  ...
  return (<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10}}>
      ...
      <select style={{...sel,width:"auto"}} value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">Tous</option>{chantiers.map(c=><option key={c.id} value={c.id}>{c.nom}</option>)}</select>
    </div>
    <div style={{background:"#fff",borderRadius:12,padding:m?12:18,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",overflowX:"auto"}}>
      {items.length === 0
        ...
        : items.map(p=>{const ch=chantiers.find(c=>c.id===(p.chantierId||p.chantier_id));const s=Math.max(0,Math.ceil((new Date(p.debut)-min)/864e5));const d=Math.max(1,Math.ceil((new Date(p.fin)-new Date(p.debut))/864e5)+1);const c=phase[ch?.phase]||"#3B82F6";
